...
def loadRSS(): 
    ...
    # saving the xml file 
    with open('indiannewsfeed.xml', 'wb') as f: 
...
def main(): 
    ...
    # parse xml file 
    newsitems = parseXML('indiannewsfeed.xml') 
  
    # store news items in a csv file 
    savetoCSV(newsitems, 'indiannews.csv') 
...
df_survey_data = pd.read_csv("indiannews.csv")
...
df.to_csv("indianheadlines.csv", mode='w', columns=COLS, index=False, encoding="utf-8")

# link file

with open("indiannews.csv", "r") as csv_file:
   csv_reader = csv.DictReader(csv_file)

   with open("indianlink.csv","w") as new_file:
    fieldnames=['link']
...
art = []
with open('indianlink.csv', 'r+') as csvfile:
...
with open('indiansummary.csv', 'w', encoding='utf-8', newline='') as file:
...
df = pd.read_csv('indianheadlines.csv')
titles = list(df.iloc[:, 0].values)
sentiment = list(df.iloc[:, 2].values)

df_link = pd.read_csv('indianlink.csv')
links = list(df_link.iloc[:, 0].values)

df_sum = pd.read_csv('indiansummary.csv')
